...
var _ builtins.Builtin = &filesystemSchema{}

func newFilesystemBuiltin(env builtins.Environment) builtins.Builtin {
	return &filesystemSchema{env}
}
...
func (s *filesystemSchema) file(p graphql.ResolveParams) (any, error) {
	...
	output, err := obj.ReadFile(p.Context, s.Buildkit(), path)
	if err != nil {
