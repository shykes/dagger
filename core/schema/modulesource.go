...
func (s *moduleSourceSchema) moduleSourceAsModule(
	...
	if bp := blueprintSrc.Self(); bp != nil {
		...
		if err := mod.ApplyLocalDefaults(ctx, getModuleDefaults(envFile, bp.ModuleName)); err != nil {
			return inst, fmt.Errorf("failed to apply local defaults for %q from %q: %q", bp.ModuleName, envFilePath, err)
		}
	}
	...
	if err := mod.ApplyLocalDefaults(ctx, getModuleDefaults(envFile, mod.Name())); err != nil {
		return inst, fmt.Errorf("failed to apply local defaults for %q from %q: %q", mod.Name(), envFilePath, err)
	}
	...
	if src := originalSrc.Self(); src.Kind == core.ModuleSourceKindLocal {
		...
		if err != nil {
			return inst, fmt.Errorf("failed to load .env from %q: %q", src.AsString(), err)
		}
		...
		if err := mod.ApplyLocalDefaults(ctx, modEnvFile.Variables()); err != nil {
			...
			return inst, fmt.Errorf("failed to apply local defaults for %q from %q: %q", mod.Name(), modEnvFilePath, err)
		}
	}
